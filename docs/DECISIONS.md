...
- 2026-09-26: Bound the desktop preview row by viewport height, and isolate the library/inspector intrinsic sizes with scroll containers. Native `details`/`summary` groups remain mounted across edits and language changes, preserving their open state and keyboard access. At narrow widths the timeline precedes tools/library.
- 2026-09-26: Add opt-in dips to black/white and linear audio fades centered on an existing cut. A documented optional v1 field references the preceding clip by ID. No source handles, overlap or retiming are invented. Shared relative transition windows drive preview and FFmpeg `fade`/`afade`; only requested filters are capability-gated. Chroma alpha is retained and titles remain independent. Filter semantics checked against [FFmpeg's filter documentation](https://ffmpeg.org/ffmpeg-filters.html#fade). True overlapping crossfades and other transition types remain future work.
